...
|----|-----------------|----------------------------------|---------------|------------|---------|
| D1 | Set `BLOG_SERVICE_URL` to the live blog-service / `ca-rust-blog` base URL (Azure App Service app settings). | `BLOG_SERVICE_URL` matches production; deploy still green. | After `ca-rust-blog` (blog-service) deploy is live. | None. | [blog-service e2e](adversarial-review/reviews/2026-04-17-blog-service-consolidation-avatar-e2e.md) |
...
| D5 | Remove `NEXT_PUBLIC_ANALYTICS_API_URL` from production env. | Grep shows no app read of that var; host config has it removed. | After `rg`/code search confirms no usage. | None. | [analytics env](adversarial-review/reviews/2026-04-14-analytics-ingestion-url-env.md) |
...
| D13 | Azure App Service: provision B1, wire GitHub OIDC, cut Cloudflare to Azure. | App Service URL live; custom domain on Cloudflare. | Before calling frontend migration complete. | Azure + Cloudflare + GitHub admin. | [app service](adversarial-review/reviews/2026-04-18-azure-app-service-frontend.md) |
...
| D24 | Set `POSTHOG_PERSONAL_API_KEY` + `POSTHOG_PROJECT_ID` in Azure App Service for my-events. | Server routes can auth to PostHog APIs in prod. | Before relying on `/api/analytics/my-events` in prod. | Secret rotation process. | [analytics-my-events](adversarial-review/reviews/2026-03-18-analytics-my-events.md) |
| D25 | Confirm frontend deploy workflow passes on `main`. | Latest workflow run green. | Next push to `main`. | None. | [deploy-prebuilt](adversarial-review/reviews/2026-03-17-deploy-prebuilt.md) |
...
- **D1 + D2** pair with env cutover + `/v1/info` rename from [blog-service consolidation](adversarial-review/reviews/2026-04-17-blog-service-consolidation-avatar-e2e.md).
