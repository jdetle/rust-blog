...
const value = process.argv[2] ?? (await prompt("Enter your Plausible domain (e.g. jdetle.com): "));
if (!value?.trim()) {
  console.error("No value provided. Usage: bun run scripts/ingest-env/plausible.ts [domain]");
  process.exit(1);
}

const trimmed = value.trim().replace(/^https?:\/\//, "").split("/")[0] ?? value.trim();
...
if (!skipVercel && (await confirm("Add to Vercel? (requires vercel CLI)"))) {
  await run("vercel", "env", "add", KEY, "production", "preview", "development", "--value", trimmed, "--yes");
