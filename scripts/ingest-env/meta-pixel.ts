#!/usr/bin/env bun
/**
 * Ingest Meta Pixel ID. Prompts for value, appends to .env.local, optionally adds to Vercel.
 * Sign up: https://business.facebook.com/events_manager → Add data source → Meta Pixel → Copy Pixel ID
 */

const KEY = "NEXT_PUBLIC_META_PIXEL_ID";
const ENV_FILE = ".env.local";

const value =
	process.argv[2] ??
	(await prompt("Paste your Meta Pixel ID (15–16 digit number): "));
if (!value?.trim()) {
	console.error(
		"No value provided. Usage: bun run scripts/ingest-env/meta-pixel.ts [PIXEL_ID]",
	);
	process.exit(1);
}
...
if (!/^\d{15,16}$/.test(trimmed)) {
	console.warn("Warning: Meta Pixel IDs are typically 15–16 digits.");
}
