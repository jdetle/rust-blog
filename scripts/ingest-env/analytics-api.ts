...
const value = process.argv[2] ?? (await prompt("Paste your analytics API URL (e.g. https://analytics-ingestion.xxx.azurecontainerapps.io): "));
if (!value?.trim()) {
  console.error("No value provided. Usage: bun run scripts/ingest-env/analytics-api.ts [URL]");
  process.exit(1);
}
