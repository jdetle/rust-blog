...
const KEY = "NEXT_PUBLIC_CLARITY_ID";
const ENV_FILE = ".env.local";

const value = process.argv[2] ?? (await prompt("Paste your Clarity Project ID: "));
if (!value?.trim()) {
  console.error("No value provided. Usage: bun run scripts/ingest-env/clarity.ts [PROJECT_ID]");
  process.exit(1);
}
...
if (!skipVercel && (await confirm("Add to Vercel? (requires vercel CLI)"))) {
  await run("vercel", "env", "add", KEY, "production", "preview", "development", "--value", value.trim(), "--yes");
